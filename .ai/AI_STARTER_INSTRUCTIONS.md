...
| Thermo library | `src/Whb.Thermo/` runtime/property libraries (own solution `Whb.Thermo.sln`), with `src/Whb.Core/Materials/Sulfur/Sulfur.fs` as the thin adapter into `XSulfur` and `src/Whb.Core/Materials/Gas/GasThermoAdapter.fs` as the thin adapter into the WhbThermo species database |
