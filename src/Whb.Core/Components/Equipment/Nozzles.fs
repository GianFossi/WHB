...
/// Dimensionamento e posizionamento dei bocchelli sul mantello:
/// discese (downcomer, acqua satura) e salite (riser, miscela bifase).
module Nozzles =

    /// Tabella tubi ASME B36.10: (NPS, ID Sch40, ID Sch80, ID Sch160) in mm
    let pipeTable =
        [ "2\"",   52.50,  49.25,  42.85
    ...
    /// Seleziona il primo diametro commerciale >= idReq
    ...
    /// Posizioni assiali ottimali di n bocchelli: baricentri di n bande
    /// a pari generazione di vapore (o a pari portata circolante).
    ...
    /// Dimensiona un set di bocchelli.
    ///   w        : portata massica totale [kg/s]
    ///   rho      : densità di riferimento [kg/m³]
    ///   vTarget  : velocità obiettivo [m/s]
    ///   rhoV2Max : limite ρv² [kg/(m·s²)]
    ///   nMax     : numero massimo di bocchelli ammesso
    let sizeSet
        ...
        // la velocita' ammessa e' la piu' restrittiva fra obiettivo e limite ρv²
    ...
    /// Posizioni sfalsate rispetto a un set dato: i downcomer vanno messi
    /// **fra** i riser, cosi' l'acqua entra dove i riser non stanno gia'
    /// estraendo, e il percorso trasversale nel mantello si accorcia.
    let staggeredPositions (n: int) (riser: float list) (l: float) =
        ...
        if List.length cand >= n then
            // sceglie n posizioni il piu' possibile equispaziate fra le candidate
            let step = float (List.length cand - 1) / float (max 1 (n - 1))
            [ for k in 0 .. n - 1 -> cand.[int (round (float k * step))] ] |> List.distinct
        else cand

    /// Riepilogo dei bocchelli EFFETTIVI, raggruppati per servizio e diametro,
    /// a partire dalle linee definite nel circuito.
    let design (case: DesignCase) (sat: Steam.SatProps) (axial: AxialResult list) (circ: CirculationResult) =
        let group (svc: string) (lines: Piping.Line list) (rho: float) (note: string) =
            ...
            |> List.map (fun (nps, ls) ->
                ...
                // portata ripartita in proporzione all'area (approssimazione per il riepilogo)
