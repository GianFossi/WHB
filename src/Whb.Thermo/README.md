...
```bash
dotnet build Whb.Thermo.sln
dotnet test  Whb.Thermo.sln
```
...
## Acqua, vapore e kernel dei gas per il solver WHB (2026-09-25)

Whb.Core non contiene più proprietà proprie di acqua, vapore o gas: le calcola qui.

- `WhbThermo.Steam.Water` — interfaccia a valori `float` per i percorsi caldi del solver:
  regioni IF97 1 e 2, saturazione (regione 4), viscosità IAPWS 2008 e conducibilità IAPWS
  2011 senza incremento critico, tensione superficiale IAPWS R1-76, record di saturazione
  `SatProps`, correlazioni ausiliarie. Usa lo stesso nucleo IF97 di `If97.properties`
...
- `WhbThermo.Radiation.GreyGas` — emissività grigia H₂O + CO₂ e coefficiente radiativo con
  parete grigia in cavità, i modelli su cui sono costruiti i risultati WHB di riferimento.

Lo spostamento non ha cambiato un solo bit dei risultati WHB, né il tempo di calcolo.

## IF97 completo e costanti fisiche uniche (2026-09-25)

- **Tutte e cinque le regioni IF97.** Regioni 3 (quasi-critica, equazione in densità con
  soluzione ρ(p, T) sul ramo corretto) e 5 (1073.15-2273.15 K, p ≤ 50 MPa) aggiunte a
  `If97.properties`; la saturazione sopra 623.15 K usa la regione 3, sia in `saturatedAt` sia
  in `Water.sat`. Coefficienti in `iapws-if97.json`, trascritti dal pacchetto `iapws` con
  `tools/import_if97_regions.py`; i test riproducono le tabelle di verifica 33 e 42 della
  release entro 10⁻⁸ e controllano l'equilibrio di fase in regione 3. Non sono implementate le
  equazioni inverse (T(p,h), T(p,s)).
- **Un solo nucleo IF97 veloce.** Regioni 1 e 2 costruiscono ogni potenza distinta una volta per
  chiamata: con questo il calcolo di cv non costa più nulla e viene sempre eseguito.
- **Costanti fisiche in un solo posto:** `WhbThermo.Domain.PhysicalConstants` (R, σ, g, punto
  critico e costante dei gas dell'acqua). `Units.Ru`, `Emissivity.sigma`, le g di Boiling,
  Convection e XSulfur e il `Constants` di Whb.Core ne sono alias; i loader IF97 e trasporto
  rifiutano un file con costanti diverse.
