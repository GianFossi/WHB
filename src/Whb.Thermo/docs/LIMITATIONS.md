...
| Stima Chung nella forma CEA, verificata su CoolProp (qualità C) | 4: C₃H₆, C₃H₈, C₆H₆, C₇H₈ |
| Sutherland dal manuale (degrada sopra ~1300 K) | 4: S₂, S₆, S₈, SO₃ |
...
### 2.5 IF97: tutte e cinque le regioni, senza equazioni inverse

Dal 2026-09-25 sono implementate le regioni 1, 2, 3, 4 e 5. La regione 3 è
un'equazione in densità: da (p, T) la densità si trova risolvendo
p(ρ, T) = p sul ramo giusto (liquido, vapore o supercritico), partendo dalle
equazioni ausiliarie SR1-86. Sopra 623.15 K (16.5 MPa) anche la saturazione
passa in regione 3, quindi un corpo cilindrico sopra i 165 bar è ora calcolato
correttamente invece che con le regioni 1 e 2 estrapolate.

Limiti che restano:

- **equazioni inverse** (T(p,h), T(p,s), v(p,T) di regione 3) non implementate:
  chi parte da (p, h) deve iterare;
- entro circa 0.01 K dal punto critico il ramo liquido e quello vapore non si
  separano più e il calcolo **fallisce esplicitamente**;
- regione 2 metastabile (vapore sottoraffreddato) non implementata.
...
Fatto il 2026-09-23; 318 test su 318 superati dal 2026-09-25. Come sono state
chiuse le quattro lacune rimaste aperte:

- **Trasporto gas di C₃H₈, C₃H₆, C₆H₆, C₇H₈.** I fit Sutherland del manuale davano
  la conducibilità fino al 55 % sotto il vero già a 800 K (CoolProp, Cantera e
  Chung concordano). Sostituiti da una stima Chung-Lee-Starling fittata nella forma
  CEA (`tools/fit_transport_chung.py`), qualità C: entro l'8 % (k) e il 2.4 % (µ) da
  CoolProp nel suo campo, entro l'11 % da Cantera fino a 1600 K per il propano.
  L'«82 %» misurato prima era in parte CoolProp estrapolato: sopra la Tmax della sua
  equazione di stato (575-725 K per questi fluidi) non è un riferimento, e
  `generate_reference.py` ora non lo usa oltre quel limite. Per lo stesso motivo il
  cp di riferimento è quello di gas ideale (`Cp0mass`).
- **Viscosità liquida dell'isobutano.** Il record DIPPR era il 18 % sopra la
  correlazione di riferimento di Vogel et al. (2000); rifittato su di essa
  (`tools/fit_liquid_reference.py`, marcato `override`, conservato da `import_dippr.py`).
- **Viscosità liquida del propilene.** Resta dubbia e marcata `suspect`: il solo
  confronto disponibile (CoolProp) è un modello a stati corrispondenti estesi, non
  una correlazione di riferimento, e non può dire chi dei due sbagli.
- **cp liquido di SO₃**: un valore singolo, ora dichiarato `singlePoint`.
- **Radicale H**: non era un difetto; i fit CEA partono da 1000 K e sotto la
  libreria estrapola con avviso. Il test di monotonia ignora ora i valori estrapolati.
...
(`binary-interaction.json`) è vuota: k_ij = 0 per ogni coppia. Per il WHB non è
una lacuna pratica: con k_ij = 0.1 su tutte le coppie del syngas di riferimento
(35 bar) il salto entalpico cambia dello 0.06 % e Z dello 0.1 %.
