...
let ``pure species properties track CoolProp`` () =
    ...
    // 64 reference points x 3 properties since the reference was limited to each
    // fluid's CoolProp Tmax (tools/generate_reference.py); the floor catches an
    // empty or misread file, not a change of coverage.
    Assert.True(compared >= 180, $"only {compared} comparisons made")
    ...
    for KeyValue(_, sp) in db do
        let values =
            ...
            |> List.choose (fun tK ->
                ...
                // Monotonicity is a property of the data inside its fitted range.
                // Outside it the library extrapolates and says so (atomic H has
                // CEA transport from 1000 K only); those values are not data.
                | Success (_, warnings) when warnings |> List.exists (function OutsideFitRange _ -> true | _ -> false) -> None
