...
let ``every correlation carries a validity range and a source`` () =
    for KeyValue(_, s) in db.Species do
        for KeyValue(name, c) in s.Correlations do
            ...
            // A single measured value is legitimate data when it says so.
            | Some lo, Some hi when c.SinglePoint ->
                Assert.True((hi = lo), $"{s.Key}/{name}: a single point needs tMin = tMax")
