...
[<Fact>]
let ``near critical state is evaluated in region 3`` () =
    // Region 3 is implemented since 2026-09-25; the reference density is the
    // iapws package value (backward equations), so 1e-5 rather than exact.
    match If97.properties model 650.0<K> 250.0<bar> with
    | Success (s, _) ->
        Assert.Equal(3, s.Region)
        Assert.True(abs (float s.Density / 488.87505207909487 - 1.0) < 1e-5, $"rho {s.Density}")
    | Failure msgs -> failwith (msgs |> List.map string |> String.concat "; ")

[<Fact>]
let ``states outside the five regions are refused`` () =
    for (t, p) in [ 1500.0<K>, 600.0<bar>; 2300.0<K>, 10.0<bar>; 700.0<K>, 1200.0<bar> ] do
        match If97.properties model t p with
        | Failure _ -> ()
        | Success (s, _) -> failwith $"({t}, {p}) must be refused, got region {s.Region}"
