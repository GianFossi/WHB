...
/// The legacy Sutherland transport of these four hydrocarbons was 20-65 % low;
/// it is now a Chung estimate. The data must grade it as an estimate (C) and
/// carry the deviations measured against CoolProp inside its range.
[<Fact>]
let ``estimated hydrocarbon transport is graded and verified in the data`` () =
    for key in [ "C3H8"; "C3H6"; "C6H6"; "C7H8" ] do
        let sp = db.[key]
        Assert.Equal(Some QualityLevel.C, sp.Quality.Transport)
        match sp.Transport with
        | NasaCea (mu, _) -> Assert.All(mu, fun iv -> Assert.StartsWith("ESTIMATE", iv.Source))
        | other -> failwith $"{key}: expected the fitted estimate, got {other}"
        match sp.Quality.Validation with
        | Verified note -> Assert.Contains("CoolProp", note)
        | other -> failwith $"{key}: expected verified, got {other}"
...
/// The fitted table must reproduce the library's own Chung module: the fit is a
/// convenience for the hot path, not a second model.
[<Fact>]
let ``fitted hydrocarbon transport reproduces the Chung module`` () =
    for key in [ "C3H8"; "C3H6"; "C6H6"; "C7H8" ] do
        let sp = db.[key]
        let c = sp.Critical.Value
        let input : WhbThermo.Properties.Chung.ChungInput =
            { Tc = c.Tc; Vc = c.Vc.Value; MolarMass = float sp.MolarMass
              Acentric = c.Acentric; DipoleMoment = 0.0; Association = 0.0 }
        for t in [ 500.0<K>; 900.0<K>; 1400.0<K> ] do
            let fitted = WhbThermo.Properties.PureComponent.viscosity sp t |> value
            let chung = WhbThermo.Properties.Chung.viscosity input t |> value
            Assert.True(abs (float fitted / float chung - 1.0) < 0.005, $"{key} mu at {t}")
