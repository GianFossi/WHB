...
def _kinds(sp: dict) -> dict[str, str | None]:
    ...
    if transport.get("kind") == "nasaCea":
        # The CEA functional form can also carry an estimate (tools/fit_transport_chung.py);
        # its sources say so, and an estimate is quality C, not B.
        sources = [iv.get("source", "") for iv in transport.get("viscosity") or []]
        estimated = bool(sources) and all(src.startswith("ESTIMATE") for src in sources)
        tkind = "estimatedFit" if estimated else "nasaCea"
    elif transport.get("kind") == "none":
        tkind = "none"
...
_LEVEL = {
    ...
    "transport": {"nasaCea": "B", "estimatedFit": "C", "sutherland": "C", "none": None},
