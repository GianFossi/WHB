...
module Nucleate =
    ...
    /// Surface tension of water against its own vapour, IAPWS R1-76, with its
    /// validity range checked. The value is Water.surfaceTension, the one
    /// implementation of the release; this adds the range check.
    let surfaceTension (t: float<K>) : Thermo<float> =
        let tc = PhysicalConstants.Water.CriticalTemperature
        let tK = float t
        if tK < 273.15 || tK > tc then
            fail (OutsideFitRange ("IAPWS R1-76", "surface tension", tK, 273.15, tc))
        else
            ok (Water.surfaceTension tK)
